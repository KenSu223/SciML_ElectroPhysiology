...
import sys
sys.path.append("/home/tianyi/SML/neuraloperator/")
...
from neuralop.losses.data_losses import H1Loss_p
...
for index in range(46, 49):
    ...
    ax.imshow(x[0], vmin=vmin, vmax=vmax)
    if index == 0: 
    ...
    im = ax.imshow(out.squeeze().detach().numpy(), vmin=vmin, vmax=vmax)
    if index == 0: 

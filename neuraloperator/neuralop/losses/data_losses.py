...
class H1Loss_p(object):
    ...
    def rel(self, x, y, h=None):
        ...
        dx = 0.01
        dy = 0.01
        left_boundary_dx = (x[:, :, 1, :] - x[:, :, 0, :]) / dx
        right_boundary_dx = (x[:, :, 31, :] - x[:, :, 30, :]) / dx
        top_boundary_dy = (x[:, :, :, 1] - x[:, :, :, 0]) / dy
        bottom_boundary_dy = (x[:, :, :, 31] - x[:, :, :, 30]) / dy
        left_boundary_dx_squared = left_boundary_dx ** 2
        right_boundary_dx_squared = right_boundary_dx ** 2
        top_boundary_dy_squared = top_boundary_dy ** 2
        bottom_boundary_dy_squared = bottom_boundary_dy ** 2
        sum_squares_dx_left = torch.sum(left_boundary_dx_squared)
        sum_squares_dx_right = torch.sum(right_boundary_dx_squared)
        total_sum_squares_dx = sum_squares_dx_left + sum_squares_dx_right
        sum_squares_dy_top = torch.sum(top_boundary_dy_squared)
        sum_squares_dy_bottom = torch.sum(bottom_boundary_dy_squared)
        total_sum_squares_dy = sum_squares_dy_top + sum_squares_dy_bottom
        total_sum_squares = total_sum_squares_dx + total_sum_squares_dy
        diff = diff + total_sum_squares
